...
export class UsersService {
  ...
  getUsers() : Observable<any>{
    ...
    return this.http.get<any>("http://3.135.206.178:5000/display",{headers:header})
    // return this.http.get<any>("http://localhost:5000/display",{headers:header})
  }

  setUsers(users:User):Observable<User[]>{
    return this.http.post<User[]>("http://3.135.206.178:5000/register",users,httpOptions);
  }

  deleteUsers(name:String):Observable<any>{
      const url="http://3.135.206.178:5000/delete/"+name;
      console.log(name);
    ...
    isExists(email: string): Observable<boolean>{
      console.log(email)
      return this.http.get<boolean>("http://3.135.206.178:5000userExists/"+ email);
    }

    updateUser(email: string, user: User): Observable<User>{
      return this.http.post<User>("http://l3.135.206.178:5000/update" + email, user);
    }
  

    userLogin(users:User):Observable<any>{
      const url="http://3.135.206.178:5000/login";
